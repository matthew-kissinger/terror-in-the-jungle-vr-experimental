...
import { Combatant, CombatantState, AISkillProfile, Faction, Squad } from './types';
...
export class CombatantSystem implements GameSystem {
  ...
  private createSkillProfile(faction: Faction, role: 'leader' | 'follower'): AISkillProfile {
    ...
    if (faction === Faction.OPFOR) {
      // OPFOR - Balanced with first shot accuracy system
      baseProfile = {
        reactionDelayMs: role === 'leader' ? 400 : 600,  // Slightly slower reactions
        aimJitterAmplitude: role === 'leader' ? 0.3 : 0.5,  // Very accurate first shot
        burstLength: role === 'leader' ? 4 : 3,  // Shorter controlled bursts
        burstPauseMs: role === 'leader' ? 800 : 1000,  // Longer reset for accuracy
        leadingErrorFactor: role === 'leader' ? 0.9 : 0.8,
        suppressionResistance: role === 'leader' ? 0.8 : 0.6,
        visualRange: 130,
        fieldOfView: 130,
        firstShotAccuracy: 0.15,  // First shot is very accurate (15% of base jitter)
        burstDegradation: 2.0  // Each shot doubles the spread
      };
    } else {
      // US forces (allies)
      baseProfile = {
        ...
        visualRange: 120,
        fieldOfView: 120,
        firstShotAccuracy: 0.2,  // Slightly less accurate first shot than OPFOR
        burstDegradation: 2.5  // Worse burst control than OPFOR
      };
    }

    // Add some randomization for variety
    baseProfile.reactionDelayMs += (Math.random() - 0.5) * 100;
    baseProfile.aimJitterAmplitude += (Math.random() - 0.5) * 0.1;  // Less random variation
  ...
  private handleEngaging(combatant: Combatant, deltaTime: number): void {
    ...
    // Determine if should go full auto
    const targetDistance = combatant.position.distanceTo(targetPos);
    combatant.isFullAuto = false;

    // Condition 1: Very close range (<15m)
    if (targetDistance < 15) {
      combatant.isFullAuto = true;
      combatant.skillProfile.burstLength = 8; // Longer bursts
      combatant.skillProfile.burstPauseMs = 200; // Shorter pauses
    }

    // Condition 2: Panic (being shot at recently)
    ...
    // Condition 3: Multiple nearby enemies
    const nearbyEnemies = this.countNearbyEnemies(combatant, 20);
    if (nearbyEnemies > 2) {
      combatant.isFullAuto = true;
      combatant.skillProfile.burstLength = 6;
    }

    // Reset to normal burst if not full auto
    if (!combatant.isFullAuto) {
      // Reset to faction defaults based on stored role
    ...
    if (!this.canSeeTarget(combatant, combatant.target)) {
      ...
      combatant.isFullAuto = true; // Full auto for suppression
  ...
  private countNearbyEnemies(combatant: Combatant, radius: number): number {
    let count = 0;
    // Check player
    if (combatant.faction === Faction.OPFOR) {
      if (combatant.position.distanceTo(this.playerPosition) < radius) {
        count++;
      }
    }
  ...
  private tryFireWeapon(combatant: Combatant): void {
    ...
    // Calculate accuracy multiplier based on burst position
    ...
    if (combatant.currentBurst === 1) {
      ...
      // Subsequent shots get progressively worse
      ...
      // Cap maximum inaccuracy
      accuracyMultiplier = Math.min(accuracyMultiplier, 4.0);
    }

    // Full auto adds even more inaccuracy
    if (combatant.isFullAuto) {
      accuracyMultiplier *= 1.5;
    }

    // Calculate shot with modified accuracy
    const shotRay = this.calculateAIShot(combatant, accuracyMultiplier);
    ...
    if (combatant.target && combatant.target.id === 'PLAYER') {
      ...
      if (playerHit.hit) {
        ...
        // Less console spam - only log significant events
        ...
        // Track misses for AI adjustment
  ...
  private calculateAIShot(combatant: Combatant, accuracyMultiplier: number = 1.0): THREE.Ray {
    if (!combatant.target) {
    ...
    // Add aim jitter with accuracy multiplier
  ...
  private applyDamage(target: Combatant, damage: number, attacker?: Combatant): void {
    ...
    // Mark that this combatant was hit (for panic/full auto decisions)
